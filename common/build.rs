//! Compiles every kernels/*.cu to PTX in OUT_DIR via nvcc (-arch=sm_61).
//!
//! Template for all stage crates. If nvcc is missing the build does not fail:
//! empty .ptx stubs are written plus a cargo:warning — the runtime code
//! reports it clearly on its own.
...
fn main() {
    ...
    for entry in fs::read_dir("kernels").expect("no kernels/ directory") {
        ...
        cmd.args(["-ptx", "-O3", "-arch", ARCH, "-lineinfo"])
            .arg(&cu)
            .arg("-o")
            .arg(&ptx);
